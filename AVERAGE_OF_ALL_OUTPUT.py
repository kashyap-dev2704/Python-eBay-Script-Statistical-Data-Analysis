...
def extractingData(Id, filepath):
    prefix = ['mtg', 'magic card', 'mtg card', 'magic']
    all_data = []
    for item in prefix:
        complete_filepath = filepath + item +'.csv'
        print('COMPLETE PATH',complete_filepath)
        with open(complete_filepath, 'r') as target:
    ...
    totaleBayAverage = [data_dict['eBayLow'] for data_dict in all_data]
...
def main():
    data = exsisting_data('K:\Snapp.ai\Final_PROJECT\DATA\demo.csv')
    #print("DATA COLLECTED",data)
    for item in data:

        path = "K:\Snapp.ai\Final_PROJECT\DATA\SOLD_CARDS\English\\"
